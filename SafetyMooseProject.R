...
TM1 <- mooseData$mYear1[c(50:nrow(mooseData))]
TM2 <- mooseData$mYear2[c(50:nrow(mooseData))] 
TM3 <- mooseData$mYear3[c(50:nrow(mooseData))] 
TM4 <- mooseData$mYear4[c(50:nrow(mooseData))]
TW <- mooseData$Wolves[c(50:nrow(mooseData))] 
TM<-mooseData$Moose[c(50:nrow(mooseData))] 
...
model15f<-update(model14f, ~.-M1:M4)
summary(model15f)
evaluateModel(model15f)
model16f<-update(model15f, ~.-M4:W)
summary(model16f)
evaluateModel(model16f)
model17f<-update(model16f, ~.-M3:W)
summary(model17f)
evaluateModel(model17f)#A whole percent!
model18f<-update(model17f, ~.-M3:M4)
summary(model18f)
evaluateModel(model18f)
model19f<-update(model18f, ~.-I(M4^2))
summary(model19f)
evaluateModel(model19f)#Most accurate model i've produce so far
model20f<-update(model19f, ~.-I(M1^2))
summary(model20f)
evaluateModel(model20f)
model21f<-update(model20f, ~.-I(M2^2))
summary(model21f)
evaluateModel(model21f)
model22f<-update(model21f, ~.-I(M3^2))
summary(model22f)
evaluateModel(model22f)
#removal of terms after this point causes decrease in accuracy
evaluateModel(model22f)
model23f<-update(model22f, ~.+M1:M3:M4:W)
summary(model23f)
evaluateModel(model23f)
model24f<-update(model23f, ~.+M2:M3:W)
summary(model24f)
evaluateModel(model24f)
#addition and removal of terms after this point causes decrease in accuracy
#seeing results of step after model24f
model25f<-step(model24f)
summary(model25f)
evaluateModel(model25f) #"Best" model
...
plot(model24f, main = "Most Accurate") #Most accurate model
plot(model25f, main="Best Branch F") #Best branch F
par(mfrow = c(1, 1)) #return plot layout to default
rpart.plot(treeModel) #best branch E
par(mfrow=c(4,4))
